...
#ifdef DEBUG
	std::cout << "dbg: GCM encrypt - key len: " << m_encryption_key.length() << " key: ";
	for(size_t i=0; i<m_encryption_key.length(); ++i)
		printf("%.2x", (unsigned char)m_encryption_key[i]);
	std::cout << " AAD len: " << (PROTOCOL_35_HEADER_SIZE - 4)
	          << " payload len: " << payloadSize << " IV: ";
